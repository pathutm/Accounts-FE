...
export default function OrgInvoiceDetailPage({ params }: { params: Promise<{ id: string }> }) {
   const { id } = use(params);
   ...
   useEffect(() => {
      const fetchInvoice = async () => {
         try {
            ...
            console.error("Failed to fetch invoice detail for org:", err);
         } finally {
            setLoading(false);
         }
      };
   ...
   const handleThreeWayMatch = async () => {
      if (!invoice.purchaseOrderId?._id) {
         alert("No PO linked to this invoice to perform 3-way match.");
         return;
      }
      ...
      try {
         ...
         // 1. Fetch aggregated match data from backend
         const resData = await fetch(`${apiUrl}/api/purchase-orders/${invoice.purchaseOrderId._id}/match-data`, {
            headers: { "Authorization": `Bearer ${token}` }
         });

         if (!resData.ok) throw new Error("Failed to fetch match data from backend");
         const matchData = await resData.json();

         // 2. Send match data to 3-way match webhook
         const webhookUrl = process.env.NEXT_PUBLIC_THREE_WAY_MATCH || 'https://api.agents.snsihub.ai/webhook/3way';
         ...
         if (resWebhook.ok) {
            alert("3-way match performed successfully!");
         } else {
            throw new Error("Webhook call failed");
         }
      } catch (err) {
         console.error("3-Way Match Error:", err);
         alert("Error performing 3-way match. Check console for details.");
      } finally {
         setMatching(false);
      }
   };
   ...
   if (loading) {
      return (
         <div className="min-h-screen flex flex-col items-center justify-center bg-[#fafafa] gap-6">
            ...
            <p className="text-xs font-black uppercase tracking-[0.3em] text-muted-foreground animate-pulse">Analyzing Payables Document...</p>
         </div>
      );
   }
   ...
   if (!invoice) {
      return (
         <div className="min-h-screen flex flex-col items-center justify-center bg-[#fafafa] gap-4">
            ...
            <button onClick={() => router.back()} className="text-sm font-bold text-primary hover:underline">Return to Accounts Payable</button>
         </div>
      );
   }
   ...
   return (
      <div className="max-w-4xl mx-auto space-y-8 animate-in fade-in slide-in-from-bottom-5 duration-1000 pb-20 font-sans mt-8">
         ...
         <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 sticky top-0 bg-background/80 backdrop-blur-sm z-50 py-4 border-b border-border/10">
            <div className="space-y-1">
               ...
               <div className="flex items-center gap-3 mt-1">
                  <h1 className="text-2xl font-black tracking-tight text-foreground inline-flex items-center gap-2 lowercase">
                     ...
                     <span className="text-xs px-3 py-1 bg-amber-500/5 text-amber-600 rounded-full border border-amber-500/10 uppercase tracking-widest">
                        {invoice.status === 'SUBMITTED' ? 'PENDING_APPROVAL' : invoice.status.replace('_', ' ')}
                     </span>
                  </h1>
               </div>
            </div>
            ...
            <div className="flex items-center gap-3">
               <button 
                  ...
                  disabled={matching}
                  className="flex items-center gap-2 px-6 py-2.5 bg-indigo-600 text-white rounded-xl text-xs font-black hover:opacity-90 transition-all shadow-xl shadow-indigo-600/20 uppercase tracking-widest disabled:opacity-50 disabled:cursor-not-allowed"
               >
                  {matching ? (
                  ...
                  <span>{matching ? 'Matching...' : 'Perform 3-Way Match'}</span>
               </button>
               <button className="flex items-center gap-2 px-6 py-2.5 bg-muted rounded-xl text-xs font-black hover:bg-muted/80 transition-all border border-border/50 shadow-sm uppercase tracking-widest text-muted-foreground">
         ...
         {/* Main Invoice Document - Mirror of Vendor side */}
         <div className="card-surface border border-border/50 shadow-2xl relative overflow-hidden bg-white min-h-[1056px]">
               ...
               <div className="space-y-6">
                  ...
                  <div className="grid grid-cols-2 gap-x-12 pt-8">
                     <div className="space-y-4">
                        <label className="text-[10px] font-black uppercase text-muted-foreground/50 tracking-[0.2em] border-l-2 border-primary/30 pl-2">Bill From (Vendor):</label>
                     ...
                     <div className="space-y-4">
                        <label className="text-[10px] font-black uppercase text-muted-foreground/50 tracking-[0.2em]">Bill To (Payer):</label>
            ...
            <div className="p-12 pt-0 flex flex-col md:flex-row justify-between gap-12">
               ...
               <div className="w-80 space-y-1 pt-6">
                  ...
                  <div className="pt-12 text-center space-y-4 font-black text-muted-foreground/30 uppercase tracking-[0.3em]">
                     Internal Verification Barcode
                  </div>
               </div>
            </div>
            ...
            <div className="p-8 border-t border-border/50 mt-12 bg-muted/5 flex items-center justify-between">
               <p className="text-[9px] font-black text-muted-foreground/30 uppercase tracking-[0.3em]">Verified Accounts Payable Department</p>
               <p className="text-[9px] font-black text-muted-foreground/30 uppercase tracking-[0.3em]">Computer Generated Approval Slip</p>
            </div>
         </div>
      </div>
   );
